    ...
    for sentence in train_dataset:
        ...
        for i in range(n):
            ...
            # Last word
                ...
                token_transition_count[y[i-1]][(x[i-1], x[i])] = token_transition_count[y[i-1]].get((x[i-1], x[i]), 0) + 1

                if bi_count.get((y[i-1], y[i])) == None:
                    bi_count[(y[i-1], y[i])] = {}
                bi_count[(y[i-1], y[i])][x[i]] = bi_count[(y[i-1], y[i])].get(x[i], 0) + 1
                bi_count[(y[i-1], y[i])][x[i]] = bi_count[(y[i-1], y[i])].get(x[i-1], 0) + 1    # added new bigram
...
def get_feature_dict_p6i(emission_count, transition_count, token_transition_count, uni_count, bi_count, state_count):
    f = {}
    ...
    for i in range(1, n):
        for k in range(len(possible_states)):
            for j in range(len(possible_states)):
                ...
                tt_key = "token_transition"+possible_states[k]+"+"+x[i-1]+"+"+x[i]
                ...
                b_key2 = "bigram: "+possible_states[j]+"+"+possible_states[j]+"+"+x[i-1]

                t_prob = f.get(t_key, -2**31)
                tt_prob = f.get(tt_key, -2**31)
                e_prob = f.get(e_key, -2**31)
                u_prob1 = f.get(u_key1, -2**31)
                # b_prob = f.get(b_key2, -2**31)
                b_prob = f.get(b_key1, 0) + f.get(b_key2, 0)
                if b_prob == 0:
                    b_prob = -2**31
                ...
                if i != n-1:
                    ...
                    overall_score = e_prob + t_prob + tt_prob + u_prob1 + u_prob2 + b_prob + scores[i-1, k]

                else:
                    overall_score = e_prob + t_prob + tt_prob + u_prob1 + b_prob + scores[i-1, k]
